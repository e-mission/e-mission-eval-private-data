...
def get_scatter(valid_users,collect_filename,first_round,second_round):
    all_filename = predict.loadModel(collect_filename)
    sc = []
    for i in range(len(valid_users)):
        df = pd.read_csv(all_filename[i], index_col='split')
        get_scatter(df, valid_users[i], i, sc, first_round, second_round)
...
if __name__ == '__main__':
    ...
    test_df = pd.read_csv("user576e37c7-ab7e-4c03-add7-02486bc3f42e.csv", index_col='split')
    test_df2 = pd.read_csv("user5079bb93-c9cf-46d7-a643-dfc86bb05605.csv", index_col='split')
    plt.figure()
    sc = []
    get_scatter(test_df,'user1',0,sc,first_round=True,second_round=None)
    get_scatter(test_df2,'user3',1,sc,first_round=True,second_round=None)
    plt.legend(sc, valid_users, markerscale=0.8, scatterpoints=1,bbox_to_anchor=(1.1,1))
